...
class Model_pelamar extends CI_Model
{
	...
	public function dataAlumni()
	{
        ...
        COUNT(IF(siswa.status='Bekerja','Benar',null)) AS bekerja,
        COUNT(IF(siswa.status='Kuliah','Benar',null)) AS kuliah,
        COUNT(IF(siswa.status='Wirausaha','Benar',null)) AS Wirausaha,
        COUNT(IF(siswa.status='Belum bekerja','Benar',null)) AS Belum_bekerja_belum_kuliah,
        COUNT(*) AS total
        FROM `siswa`
	...
	{
		$sql = "SELECT tahun_ajaran.tahun_ajaran,
 COUNT(IF(siswa.status='Bekerja','Benar',null)) AS bekerja,
        COUNT(IF(siswa.status='Kuliah','Benar',null)) AS kuliah,
        COUNT(IF(siswa.status='Wirausaha','Benar',null)) AS Wirausaha,
        COUNT(IF(siswa.status='Belum bekerja','Benar',null)) AS Belum_bekerja_belum_kuliah,
...
ORDER BY siswa.id ASC;";
	...
	public function homeBelumBekerja()
	{
        ...
        INNER JOIN tahun_ajaran
        ON siswa.tahun_lulus=tahun_ajaran.id_tahun_ajaran
        WHERE tahun_ajaran.status='AKTIF' AND siswa.status='Belum bekerja';";
		$query = $this->db->query($sql);
		return $query->row_array();
	}
